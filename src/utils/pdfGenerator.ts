...
export const generateSimplePDF = (resumeData: ResumeData): void => {
  ...
  pdf.setFontSize(11);
  pdf.setFont(undefined as any, 'normal');
  yPosition = addText(resumeData.personalInfo.summary, 20, yPosition + 5);
  ...
  resumeData.education.forEach((edu) => {
    ...
    pdf.setFontSize(12);
    pdf.setFont(undefined as any, 'bold');
    yPosition = addText(`${edu.degree} in ${edu.field}`, 20, yPosition + 8);
  ...
  Object.entries(skillsByCategory).forEach(([category, skills]) => {
    pdf.setFontSize(11);
    pdf.setFont(undefined as any, 'bold');
    yPosition = addText(`${category.charAt(0).toUpperCase() + category.slice(1)}:`, 20, yPosition + 6);
